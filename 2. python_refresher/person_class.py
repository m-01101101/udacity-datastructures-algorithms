"""Let's assume that the current month is April, 
and you want to use a Person class to help make use of information about the friends in your contacts list. 
...
Finally, you would also like to figure out which month has the most friends with birthdays, 
so you can budget for all of the birthday cards you will need to buy.
"""
...
def get_april_birthdays(people: list) -> dict:
    ...
    # all people w April birthdays as keys, and their updated ages as values
    april_birthdays = {}
    for p in [p for p in people if p.birthday_month == "April"]:
        p.birthday()
        april_birthdays.update({p.name: p.age})
...
def get_most_common_month(people):
    ...
    # good way to update existing dict
    # for person in people:
    #     months[person.birthday_month] += 1

    birthdays = [p.birthday_month for p in people]
    months = Counter(birthdays)
    # returns a list with single tuple, want just the month
